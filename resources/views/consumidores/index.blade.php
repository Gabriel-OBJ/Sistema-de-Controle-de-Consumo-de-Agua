                                ...
                                <tr>
                                    ...
                                    <td class="text-end pe-4">
                                        @if($consumidor->leituras()->exists())
                                            {{-- Consumidor com histórico: remoção bloqueada (LGPD / integridade financeira) --}}
                                            <button class="btn btn-sm btn-outline-secondary" disabled
                                                title="Não é possível remover: consumidor possui histórico de leituras">
                                                <i class="bi bi-lock-fill"></i>
                                            </button>
                                        @else
                                            {{-- Consumidor sem histórico: remoção permitida --}}
                                            <form action="{{ route('consumidores.destroy', $consumidor) }}" method="POST"
                                                class="d-inline"
